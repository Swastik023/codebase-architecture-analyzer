# Code Cartographer JSON IR Schemas
...
explicit authored viewBox without overlap, Code Cartographer omits the complete legend
...
by `code-cartographer brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `code-cartographer brands capture <url> --json`. Known IDs and known-brand
...
can be produced explicitly with `code-cartographer migrate workflow ... --to-schema 2`.
...
strictly Code Cartographer judges composition. `engineering_profile` is an optional
