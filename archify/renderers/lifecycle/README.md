...
Render `diagram_type: "lifecycle"` JSON files into the standard Code Cartographer HTML
template.

```bash
node code-cartographer/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `code-cartographer/schemas/lifecycle.schema.json`
...
`code-cartographer/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
code-cartographer/schemas/lifecycle.schema.json
```
