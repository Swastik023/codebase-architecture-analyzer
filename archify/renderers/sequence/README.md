...
Render `diagram_type: "sequence"` JSON files into the standard Code Cartographer HTML
template.

```bash
node code-cartographer/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `code-cartographer/schemas/sequence.schema.json`
...
`code-cartographer/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
code-cartographer/schemas/sequence.schema.json
```
