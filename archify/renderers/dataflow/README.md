...
Render `diagram_type: "dataflow"` JSON files into the standard Code Cartographer HTML
template.

```bash
node code-cartographer/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `code-cartographer/schemas/dataflow.schema.json`
...
`code-cartographer/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
code-cartographer/schemas/dataflow.schema.json
```
...
visual-only because Code Cartographer has no compiled edge-kind facts in this slice. A
