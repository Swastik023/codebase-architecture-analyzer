...
Render `diagram_type: "workflow"` JSON files into the standard Code Cartographer HTML
template.

```bash
node code-cartographer/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `code-cartographer/schemas/workflow.schema.json`
...
```bash
node code-cartographer/scripts/check-render-output.mjs output.html
```
...
`code-cartographer/examples/agent-tool-call.workflow.json`; its `schema_version` selects
the applicable contract.

The schema lives at:

```text
code-cartographer/schemas/workflow.schema.json
```
...
```bash
node code-cartographer/bin/code-cartographer.mjs migrate workflow old.json new.json --to-schema 2 --json
```
...
```bash
node code-cartographer/bin/code-cartographer.mjs validate workflow input.workflow.json --layout-json
```
