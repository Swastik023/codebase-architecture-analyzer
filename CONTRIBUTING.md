...
Thank you for helping Code Cartographer make engineering diagrams more trustworthy and useful. Code Cartographer is Agent-first: people describe the system they want to explain, while the Skill, typed JSON contract, renderers, validators, and delivery receipts make the result reproducible.
...
Code Cartographer's public behavior is larger than a renderer function. Treat these as contracts:
...
The renderer package lives in `code-cartographer/` and supports Node.js 18 and later. CI covers Node.js 18, 20, 22, and 24.
...
Test public behavior through a supported seam whenever possible: `code-cartographer render`, `validate`, `deliver`, `visual-check`, or the final SVG/HTML. Private helper tests are useful for edge cases, but they do not replace a CLI or artifact-level regression.
...
3. Run `npm test` from `code-cartographer/`.
...
A browser test that was skipped because Chrome was unavailable is **skipped**, not passed. Report automated browser evidence independently from perceptual visual review. Follow the [delivery contract](code-cartographer/references/delivery-contract.md) when recording supplementary manual browser work; an unconstrained glance supports only perceptual review.

### CLI, receipts, and delivery

Preserve non-zero exit behavior and machine-readable receipts. A successful `validate` does not prove atomic delivery, and a successful `deliver` does not prove perceptual quality. Follow [the delivery contract](code-cartographer/references/delivery-contract.md) and test the failure stage you changed.
...
scripts/build-zip.sh /tmp/code-cartographer-contrib.zip
```

The runtime follows the Node range in `code-cartographer/package.json`, but canonical
`code-cartographer.zip` container bytes are built only with Node 22. The builder rejects
other Node majors so a different bundled zlib cannot publish a second byte
representation of the same package contents.

Bundled example or viewer changes normally require the Gallery rebuild. Skill runtime, schema, renderer, or published `SKILL.md` changes require checking `code-cartographer.zip` freshness and committing a rebuilt archive when the checked-in package contents differ.
...
1. The exact Code Cartographer version or commit, installation method, command, and environment.
...
Showcase cases should be reproducible proof, not promotional screenshots. Submit the original prompt, agent/client, exact model, Code Cartographer version, redacted typed JSON, artifact, validation receipt, and truthful visual-review status through `.github/ISSUE_TEMPLATE/showcase.yml`.
