...
These are generated Code Cartographer artifacts, not product mockups. Click a frame to open its live, shareable state.

<p align="center">
  <a href="https://tt-a1i.github.io/code-cartographer/gallery.html"><img src="docs/assets/code-cartographer-live-proof.gif" alt="Three verified Code Cartographer artifacts moving through Signal Flow, Blueprint, and Classic presets" width="960"/></a>
  <br/>
  <sub><strong>Three real generated artifacts.</strong> Signal Flow · Blueprint · Classic · <a href="https://tt-a1i.github.io/code-cartographer/gallery.html">open the interactive Proof Lab ↗</a></sub>
</p>

| Guided story | Route probe | Semantic lens |
|---|---|---|
| [![Agent workflow playing one authored chapter](docs/assets/code-cartographer-demo-story.png)](https://tt-a1i.github.io/code-cartographer/gallery/artifacts/agent-tool-call.workflow.html?theme=dark&present=1&play=1#view=happy-path) | [![Cache-miss sequence showing the Web App to Postgres route](docs/assets/code-cartographer-demo-route.png)](https://tt-a1i.github.io/code-cartographer/gallery/artifacts/cache-miss.sequence.html?theme=dark&present=1#route=web~db) | [![Production architecture comparing backend and database roles](docs/assets/code-cartographer-demo-lens.png)](https://tt-a1i.github.io/code-cartographer/gallery/artifacts/production-deployment.architecture.html?theme=dark&present=1#lens=backend~database) |
| Play one finite named chapter. | Inspect the shortest authored directed path. | Compare real traffic between semantic roles. |

The [Proof Lab](https://tt-a1i.github.io/code-cartographer/gallery.html) contains all 11 checked-in scenarios, their JSON sources, named views, and validation receipts.

### A real repository, mapped from source

[![MCO runtime architecture generated from the public mco-org/mco repository](docs/assets/mco-runtime-share-card.png)](https://tt-a1i.github.io/code-cartographer/cases/mco-runtime.architecture.html?theme=dark&present=1#view=dispatch-path)

Code Cartographer traced [`mco-org/mco`](https://github.com/mco-org/mco) at `9f1a1cf` and produced this checked map. **[Open it ↗](https://tt-a1i.github.io/code-cartographer/cases/mco-runtime.architecture.html?theme=dark&present=1#view=dispatch-path)** · [trace reach ↗](https://tt-a1i.github.io/code-cartographer/cases/mco-runtime.architecture.html?theme=dark#focus=router&reach=downstream) · [typed source](docs/cases/mco-runtime.architecture.json)
...
| Dark | Light |
|---|---|
| ![Dark theme](docs/assets/code-cartographer-dark.png) | ![Light theme](docs/assets/code-cartographer-light.png) |

The Export menu copies PNG to the clipboard and downloads static or motion formats:

![Export menu](docs/assets/code-cartographer-menu.png)
...
![Route Share Card showing the exact Users to API Server path with the full architecture retained as context](docs/assets/code-cartographer-route-share-card.png)
...
[DSH community opt-in](integrations/deepseek-harness/README.md): `dsh plugin --profile web add @tt-a1i/code-cartographer-dsh@0.1.0`

The [agent switcher](https://tt-a1i.github.io/code-cartographer/start.html?agent=cursor&type=architecture) covers `cursor`, `codex`, `claude-code`, and `opencode`. For Raven's manual ZIP install, extract [`code-cartographer.zip`](code-cartographer.zip) into `~/.raven/workspace/skills`; it yields `~/.raven/workspace/skills/code-cartographer`. Raven is not a switcher target.

Code Cartographer may GET the fixed stable manifest solely to show an optional reminder; it never downloads or installs updates. Successful checks wait about 72 hours (±20%); active use retries failures after 6, then 24 hours. The server sees normal HTTP metadata (IP and time), but receives no version, Agent, project data, prompts, account/device ID, or ETag. You decide whether and when to update. Set `ARCHIFY_UPDATE_CHECK_DISABLED=1` to disable networking and reminder-state writes.

### 2. Start from a description — no repository required

```text
Use Code Cartographer to draw: Browser -> API -> Redis cache -> PostgreSQL fallback.
```

For source evidence, open a repository and ask:

```text
Analyze this repository, then use code-cartographer to create a high-level runtime architecture diagram.
...
Continue with focused requests such as `add Redis`, `move auth to the left`, or `highlight the rollback path`. Code Cartographer keeps the typed source available for targeted iteration.
...
Not sure which one fits? Use the [interactive scenario guide](https://tt-a1i.github.io/code-cartographer/guide.html), or ask the zero-dependency CLI:

```bash
node code-cartographer/bin/code-cartographer.mjs guide "Show an API request with Redis cache miss"
node code-cartographer/bin/code-cartographer.mjs guide "Map Kafka topics, consumer groups, replay, and DLQ" --json
```

Workflow keeps the happy path clear across lanes:

![Workflow example](docs/assets/code-cartographer-workflow.png)

Sequence explains one interaction over time:

![Sequence example](docs/assets/code-cartographer-sequence.png)

Data Flow makes movement and sensitivity boundaries explicit:

![Data Flow example](docs/assets/code-cartographer-dataflow.png)

Lifecycle separates progress, waits, retries, and terminal outcomes:

![Lifecycle example](docs/assets/code-cartographer-lifecycle.png)

Architecture examples: [`web-app`](examples/web-app.html) · [`Code Cartographer pipeline`](examples/code-cartographer-repo.html) · [`grid placement`](examples/code-cartographer-repo-grid.html) · [`desktop agent`](examples/maka-architecture.html)
...
Code Cartographer is not a general-purpose drawing editor or a Mermaid theme. It turns technical intent into a communication artifact.
...
node bin/code-cartographer.mjs guide "Show CI/CD checks, approval, deploy, and rollback"
...
The complete generation and viewer contract lives in [`code-cartographer/SKILL.md`](code-cartographer/SKILL.md).

## Installation options

| Surface | Install location or method | Capability |
|---|---|---|
| **Raven** | Manual ZIP into `~/.raven/workspace/skills` → `~/.raven/workspace/skills/code-cartographer` | Full renderer + validation workflow |
...
| **Claude.ai** | Upload `code-cartographer.zip` under Settings → Capabilities → Skills | Depends on Node.js access in the sandbox |
| **Project Knowledge** | Upload `code-cartographer.zip` to the project | Prompt-driven architecture fallback |
| **DeepSeek Harness** | Opt-in: `dsh plugin --profile web add @tt-a1i/code-cartographer-dsh@0.1.0`. Invoke: `Use the code-cartographer skill to map this repository's runtime architecture.` Remove: `dsh plugin --profile web remove @tt-a1i/code-cartographer-dsh`. | Community integration for developer-preview `@deepseek-ai/dsh@0.1.0-rc.6`; Node `^22.19.0 \|\| >=24.0.0`; not an official DeepSeek product. No telemetry. Shell files need exact workspace paths, not Web Produced Files. [Details](integrations/deepseek-harness/README.md). |

## Reference and scope

- [Schema reference](code-cartographer/schemas/README.md) · [Skill](code-cartographer/SKILL.md) · [Examples](code-cartographer/examples/) · [Agent cookbook](docs/authoring-cookbook.md)
- [Changelog](CHANGELOG.md)
- [Roadmap](ROADMAP.md)
- [Generated Proof Lab](https://tt-a1i.github.io/code-cartographer/gallery.html)
...
Issues, pull requests, and real-world diagrams are welcome. Start with the [contribution guide](CONTRIBUTING.md), use the reproducible bug form for failures, or submit a validated diagram through the [community showcase form](https://github.com/tt-a1i/code-cartographer/issues/new?template=showcase.yml).&nbsp;·&nbsp;[LINUX&nbsp;DO](https://linux.do)
