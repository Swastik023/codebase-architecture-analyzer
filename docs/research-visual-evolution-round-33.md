...
Code Cartographer can already restore a named chapter, one focused node, one authored route,
...
[current shared viewer source](../code-cartographer/assets/template.html)).
...
resolved stop inside that projection. On load, Code Cartographer rebuilds the current node,
...
[viewer URL writers and readers](../code-cartographer/assets/template.html)):
...
([focus writer](../code-cartographer/assets/template.html),
[guided-view writer](../code-cartographer/assets/template.html),
[route writer](../code-cartographer/assets/template.html),
[Lens writer](../code-cartographer/assets/template.html)).
...
[current Story Beat implementation](../code-cartographer/assets/template.html)).
...
([Semantic Camera and export implementation](../code-cartographer/assets/template.html)).
...
**Adapt:** Code Cartographer's exact Story moment needs one narrower coordinate inside that
...
**Adapt:** use a stable node ID instead of a numeric fragment index. Code Cartographer node
...
coordinates into an Code Cartographer link. In React Flow those values are the editable
flow; in Code Cartographer the JSON IR and compiled SVG are canonical, and runtime state
...
breakpoint, embed size, browser zoom, or Presentation chrome. Code Cartographer can instead
derive an appropriate camera from the same semantic chapter nodes on the current
device. This is an Code Cartographer design inference based on React Flow's explicitly
pixel/zoom viewport model and Code Cartographer's current semantic camera implementation
([React Flow viewport definition](https://reactflow.dev/learn/concepts/terms-and-definitions#viewport),
[Code Cartographer camera source](../code-cartographer/assets/template.html)).
...
**Adapt:** Code Cartographer's view is already the board and its existing stable node ID is
...
**Adapt:** Code Cartographer can go one useful step further and document the story beat as a
...
| Tool | Stable semantic address | Exact incremental position | Raw viewport snapshot | Embed/autoplay meaning | Code Cartographer judgment |
|---|---|---|---|---|---|
...
| Code Cartographer now | `focus`, `route`, `lens`, `view` | Beat is in memory only | Derived camera, no URL snapshot | `play=1` is bounded chapter scope | Add `view + beat`, preserve everything else |
