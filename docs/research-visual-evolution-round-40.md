...
Build **First-fold Proof Aperture**: make a meaningful slice of Code Cartographer's
...
Code Cartographer already has the richer product: three live proof choices, five typed
...
because it is visible proof, not because Code Cartographer must copy twelve styles or a
GIF renderer. Adding another interaction to an already dense viewer would be
imitating surface breadth. Making Code Cartographer's own stronger artifact visible is
...
[`code-cartographer/test/landing.test.mjs`](../code-cartographer/test/landing.test.mjs) verifies that
...
`docs/assets/code-cartographer-live-proof.gif`, backed by a receipt and focused tests.
...
**Adapt:** expose Code Cartographer's existing real HTML proof rather than copying a
...
**Adapt:** Code Cartographer already has a richer interactive artifact, so reveal it
...
preview.
**Adapt:** keep Code Cartographer's current three proof tabs and full artifact link.
...
**Adapt:** retain Code Cartographer's read-only, generated artifact and keyboard proof
tabs.
...
| Adapt | Reuse the three checked Code Cartographer artifacts and existing tab semantics. |
...
Code Cartographer does not need another capability to become more convincing in this
...
and let the canvas own attention—while preserving Code Cartographer's narrower, stronger
