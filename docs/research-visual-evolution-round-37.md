...
Code Cartographer already has the major viewer primitives that modern graph tools use to
...
([Guided Story implementation](../code-cartographer/assets/template.html),
[Semantic Camera implementation](../code-cartographer/assets/template.html)).
...
first-glance hierarchy and reading comprehension—and what should Code Cartographer add
after its existing Radar, Camera, Reading Depth, Guided Story, Route Journey,
Motion Governor, and presentation features?

This note uses:

1. the current Code Cartographer worktree and generated Proof Lab artifacts;
...
4. explicit product inferences where a source supplies a primitive rather than
   an Code Cartographer-specific recommendation.
...
## Current Code Cartographer baseline: do not rebuild solved layers
...
([Semantic Radar implementation](../code-cartographer/assets/template.html),
[Semantic Radar tests](../code-cartographer/test/semantic-radar.test.mjs)).
...
([shared viewer CSS and camera](../code-cartographer/assets/template.html)).
...
([Reading Depth implementation](../code-cartographer/assets/template.html),
...
position means “default opening.” Code Cartographer should not infer cold-open intent from
...
([Story Beat tests](../code-cartographer/test/story-beat-navigator.test.mjs),
...
([camera implementation](../code-cartographer/assets/template.html),
[Route Journey tests](../code-cartographer/test/route-journey.test.mjs)).
...
Code Cartographer's existing camera. It should not store a React Flow viewport or make the
...
**Adapt:** Code Cartographer should not queue camera moves ahead of the reader. The Story
...
**Adapt:** Code Cartographer already has camera transaction IDs and settlement receipts.
...
**Adapt:** Code Cartographer's authored order is already `meta.views[].focus`; its Story
...
**Adapt:** Code Cartographer should preserve one SVG and express past/current/pending by
...
> Play or select a Story Beat, and Code Cartographer follows that exact authored moment
...
Call the already-public `Code Cartographer.view.reveal(ids, options)` from a small
...
- one shared Story-to-Camera request path in
  [`code-cartographer/assets/template.html`](../code-cartographer/assets/template.html);
- one story-owned caller of the existing public `Code Cartographer.view.reveal` method,
...
**Adapt** them into a Story Follow Camera over Code Cartographer's existing stable IDs,
...
observed legibility failure at the exact point Code Cartographer's narrative becomes most
