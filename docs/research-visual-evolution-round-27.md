...
Code Cartographer already has two category surfaces:
...
> the existing Semantic Lens, and can that feel alive without turning Code Cartographer
...
[`Code Cartographer.semanticLens`](../code-cartographer/assets/template.html) currently:
...
- [`render-architecture.mjs`](../code-cartographer/renderers/architecture/render-architecture.mjs)
  ...
  bridge.
- [`render-workflow.mjs`](../code-cartographer/renderers/workflow/render-workflow.mjs)
...
- [`render-lifecycle.mjs`](../code-cartographer/renderers/lifecycle/render-lifecycle.mjs)
...
- [`render-sequence.mjs`](../code-cartographer/renderers/sequence/render-sequence.mjs)
  explains message/edge variants such as request and return. It is not a node
  category legend and must remain static.
- [`render-dataflow.mjs`](../code-cartographer/renderers/dataflow/render-dataflow.mjs)
...
of the live SVG for Code Cartographer's export path. Code Cartographer already has a stronger
canonical-clone boundary and should keep runtime decorations out of export.

This source is also a useful negative test: a legend can look like Code Cartographer's
...
The exact count badge is an Code Cartographer inference from the same category-to-ID
...
Code Cartographer must add keyboard, focus, touch, semantic state, and target-size behavior
...
This distinction maps cleanly to Code Cartographer:
...
For Code Cartographer's SVG-hosted legend rows, this means:
...
state. Code Cartographer should borrow it, with stricter limits:
...
4. The shared viewer upgrades marked rows at runtime. It derives counts from the
   ...
   to `Code Cartographer.semanticLens`.
...
- G6's exact category-to-element mapping, implemented through Code Cartographer's existing
...
- Edge-semantic legend lenses for Sequence and Data Flow, but only after Code Cartographer
...
3. Every visible count equals the corresponding value returned by
   `Code Cartographer.semanticLens.kinds()` after deduplication by `data-node-id`.
