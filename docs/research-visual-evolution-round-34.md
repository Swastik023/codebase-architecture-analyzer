...
Code Cartographer already exposes authored relationships in every typed renderer and can
...
([relationship renderer contract](../code-cartographer/renderers/shared/cli.mjs),
[current Relationship Lens](../code-cartographer/assets/template.html),
[current relationship tests](../code-cartographer/test/relationship-lens.test.mjs)).
...
orientation interaction. Code Cartographer already has node focus, one-hop intent,
...
> Should an Code Cartographer reader be able to interact with an authored relationship
...
of 2026-07-20. Product conclusions and proposed Code Cartographer behavior are explicitly
marked as recommendations or inferences rather than vendor claims.

## Current Code Cartographer baseline
...
([shared renderer helper](../code-cartographer/renderers/shared/cli.mjs)). The current
...
([relationship renderer tests](../code-cartographer/test/relationship-lens.test.mjs)).
...
[shared node accessibility helper](../code-cartographer/renderers/shared/cli.mjs)).
...
[current relationship geometry helper](../code-cartographer/assets/template.html)).
...
row visible
([Relationship Lens implementation](../code-cartographer/assets/template.html),
[Relationship Lens tests](../code-cartographer/test/relationship-lens.test.mjs)).

The gap is consequently not “invent an edge inspector.” It is “let the line open
the inspector state Code Cartographer already has.” Reusing the existing panel is important
...
embed variants
([shared viewer chrome](../code-cartographer/assets/template.html)).
...
([preview and pulse implementation](../code-cartographer/assets/template.html),
[export-clean regression tests](../code-cartographer/test/relationship-lens.test.mjs),
[pulse regression tests](../code-cartographer/test/relationship-pulse.test.mjs)).
...
channel
([Motion Governor implementation](../code-cartographer/assets/template.html),
[Motion Governor tests](../code-cartographer/test/motion-governor.test.mjs)).
...
([focus, route, Lens, and Guided Story hash readers](../code-cartographer/assets/template.html),
...
([shared renderer helper](../code-cartographer/renderers/shared/cli.mjs),
[typed renderer examples](../code-cartographer/examples)). Therefore Round 34 should keep
...
**Adapt:** Code Cartographer is a viewer, so activation means “inspect/pin this authored
...
contract is clearer for Code Cartographer.
...
**Adapt:** Code Cartographer already has SVG and can create genuine focusable DOM targets,
...
**Adapt:** Code Cartographer needs one pinned relationship, not yFiles' editor command
...
radius. This is an Code Cartographer implementation recommendation based on the W3C and
...
Code Cartographer should borrow the focus-management technique, not claim that diagram
...
**Borrow later:** if Code Cartographer adds relationship links, use an author-controlled
...
| Should pin create new UI? | Code Cartographer already has an exact Relationship Lens row and is chrome-dense. | No. Reuse and contextualize the existing Lens. |
...
| Is current identity sufficient for a deep link? | Graphviz requires author-controlled unique IDs; Code Cartographer's key is a source-order number. | No. Keep the pin local in Round 34 and do not emit a relation URL. |
...
- do not add a schema field, migration, server, storage, dependency, or editor
  API; and
- expose at most a small read-only `Code Cartographer.relationship` receipt/API for testing
...
works with mouse, touch, and keyboard, and composes with Code Cartographer's existing
