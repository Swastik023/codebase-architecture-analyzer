...
Code Cartographer now has enough good motion primitives. Its next visual-quality problem is
...
- the current `codex/code-cartographer-visual-evolution` worktree, not an older package;
...
- the existing Code Cartographer boundary: one deterministic self-contained HTML artifact,
...
## Current Code Cartographer evidence
...
[header and preset styling](../code-cartographer/assets/template.html#L519-L632),
[Radar styling](../code-cartographer/assets/template.html#L705-L752), and
[motion rules](../code-cartographer/assets/template.html#L2635-L2936).
...
- [ambient trace pause selectors](../code-cartographer/assets/template.html#L2933-L2936);
- [manual Story start](../code-cartographer/assets/template.html#L5467-L5483);
- [chapter activation and Semantic Camera reveal](../code-cartographer/assets/template.html#L5536-L5555).
...
See the current [reduced-motion block](../code-cartographer/assets/template.html#L2998-L3047)
...
may throttle them, but Code Cartographer has not expressed an application invariant.
...
### The existing `Code Cartographer.motion` name is already taken

`Code Cartographer.motion` is the public WebM recording surface with `canRecord()` and
`recordWebm()`. A governor must not replace or silently change this API. Use a
distinct owner such as `Code Cartographer.motionGovernor` and keep explicit motion export
separate from live-viewer preference.

See [the current recording implementation](../code-cartographer/assets/template.html#L3920-L4025)
and [animation contract tests](../code-cartographer/test/animation.test.mjs).
...
WAI Technique G186 is unusually direct for Code Cartographer: the control must be
...
It is Level AAA, but it matches Code Cartographer's stated quality target and existing
...
Code Cartographer's interactive artifact can offer reader control that a GIF cannot.
...
second persistent camera model. Code Cartographer's current 480ms bounded fit is enough.
...
**Skip now:** adding this extra movement before Code Cartographer can guarantee that
...
```js
Code Cartographer.motionGovernor = {
  mode(),
  owner(),
...
Keep the existing `Code Cartographer.motion.canRecord()` and
`Code Cartographer.motion.recordWebm()` export surface untouched.
...
8. `Code Cartographer.motionGovernor` exists without replacing or renaming
   `Code Cartographer.motion` WebM methods.
...
47. Explicit WebM recording remains available for trace artifacts through the
    existing `Code Cartographer.motion` API and contains no Governor UI/state.
