...
- [`relationshipTokenKind()` and `relationshipTokenGeometry()`](../code-cartographer/assets/template.html)
...
- [`storyStep()` and `pulseStoryStep()`](../code-cartographer/assets/template.html) build
  ordered Story Beats and animate the generic Story Trail flow;
- [`storyMotionAllowed()`](../code-cartographer/assets/template.html) rejects every embed,
  even when the URL explicitly requested the existing `?embed=1&play=1` share
  contract;
- [`followStoryStep()`](../code-cartographer/assets/template.html), by contrast, correctly
...
- [shared Story Beat implementation](../code-cartographer/assets/template.html)
...
**Adapt:** Code Cartographer already has stronger stable `data-edge-key` / optional
...
**Adapt:** one carrier per selected Story Beat, with Code Cartographer's existing cleanup
and motion governor.
