...
Code Cartographer's desired quality bar, or an aggregate score that cannot name a route.
...
to routing quality.
**Adapt:** compute Code Cartographer's `cross-container` class from typed structural
...
**Adapt:** use discrete explainable Code Cartographer classes instead of exposing router
...
**Adapt:** make ordinary Code Cartographer sequence messages an exact renderer invariant
...
**Adapt:** reuse Code Cartographer's existing small `role` vocabulary instead of adding a
...
**Code Cartographer keeps primary routes direct, gives necessary detours an explicit
...
The 8px terminal threshold is an Code Cartographer adaptation below the current 13px
...
- Desktop/mobile have no horizontal overflow or clipped receipt text; focus
  targets remain at least 44px; console contains no Code Cartographer runtime errors.
