...
copy Fireworks' complete showcase budget into Code Cartographer. Instead, it should move
...
This is deliberately a correctness gate, not an auto-router. Code Cartographer keeps the
...
- `code-cartographer/renderers/shared/geometry.mjs`: `segmentIntersectsRect` already
  provides the shared segment/rectangle primitive;
- `code-cartographer/renderers/workflow/render-workflow.mjs`: the only renderer that
  currently expands unrelated node boxes by 2px and rejects a crossing;
- `code-cartographer/renderers/architecture/render-architecture.mjs`,
...
- `code-cartographer/scripts/check-render-output.mjs`: performs a separate, post-render
...
`code-cartographer/test/layout-rules.test.mjs` has a focused negative fixture for a
workflow edge crossing a non-endpoint node, and
`code-cartographer/test/geometry.test.mjs` directly tests the segment/rectangle
...
**Adapt:** begin with the one invariant Code Cartographer can infer exactly from its
...
quality profile, not an unconditional retrofit over Code Cartographer's existing
...
classes.
**Adapt:** Code Cartographer should hard-gate peer node boxes and explicitly exempt
...
**Adapt:** retain Code Cartographer's current 2px workflow clearance in the first shared
gate so the rollout fixes inconsistency without silently tightening every
existing layout.
**Skip:** claiming a global zero-crossing guarantee when Code Cartographer is not running
...
message.
**Adapt:** Code Cartographer's receipt should point directly to `via`, `route`,
...
Code Cartographer currently has no schema concept for a junction, shared corridor,
...
**Code Cartographer never publishes a relationship through an unrelated semantic node,
...
exempt because crossing them can be semantically necessary and Code Cartographer has no
...
4. `code-cartographer validate <type> <input>` and direct renderer invocation reject the
   same bad geometry.
...
| Adapt | Reuse Code Cartographer's exact authored route points and the existing 2px workflow clearance. |
...
the right improvement is the small, strict statement Code Cartographer can prove today:
