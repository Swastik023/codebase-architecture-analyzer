...
Code Cartographer already turns each authored `meta.views[].focus` list into a visible Story
...
## Why this slice is real in current Code Cartographer

The implementation substrate already exists in
[`Code Cartographer.guidedViews`](../code-cartographer/assets/template.html):
...
`docs/gallery/artifacts/*.html`, using only `code-cartographer-guided-views-data` and exact
...
request one finite signal over one exact Code Cartographer path, while the static beat
...
**Adapt:** Code Cartographer already has an authored stable-ID node sequence, a fixed SVG,
...
state, and previous/next semantics. **Adapt:** Code Cartographer's authored `focus` order is
...
recomputation, and any claim that prior Code Cartographer beats mutate the graph.
...
14. Expose a read-only deterministic receipt such as
    `Code Cartographer.guidedViews.beat()` returning `{index, total, nodeId, relation,
