...
Code Cartographer has strong committed exploration after a reader clicks a node: Semantic
...
Can Code Cartographer make the diagram feel immediately explorable without adding another
...
`clickNode`. The event payload includes the stable node ID. Code Cartographer should keep
...
and notes that labels or details can be delayed until tap/mouseover. Code Cartographer
...
triggered it. Code Cartographer already has the equivalent stable identity in
