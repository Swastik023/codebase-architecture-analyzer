...
Code Cartographer is primarily an Agent-facing Skill. Normal users can ask a Skill-capable Agent to generate the diagram; they do not need to learn the schema or run `validate`, `inspect`, or `deliver` themselves.

The manual workflow below is a reference for integration, contribution, and troubleshooting. The commands assume that you are in the repository's `code-cartographer/` directory, or in the root of an installed Code Cartographer skill.

## 1. Check the installation

Code Cartographer requires Node.js 18 or later. Run the doctor command before authoring a diagram:
...
```bash
node bin/code-cartographer.mjs guide "Show an API request with a Redis cache miss" --json
```
...
Every source needs a `schema_version`, a `diagram_type`, a `meta.title`, and the structural arrays required by that renderer. Read the [schema reference](../code-cartographer/schemas/README.md) for the exact fields and allowed values.
...
```bash
node bin/code-cartographer.mjs validate architecture path/to/diagram.json \
  --repo-root path/to/repository --quality showcase --json
```

Code Cartographer verifies the Git origin, commit, blobs, and requested lines. Do not add source evidence when the repository or revision cannot be verified.
...
```bash
node bin/code-cartographer.mjs validate architecture examples/web-app.architecture.json \
...
```bash
node bin/code-cartographer.mjs inspect architecture path/to/diagram.json
```
...
```bash
node bin/code-cartographer.mjs deliver architecture examples/web-app.architecture.json \
...
```bash
node bin/code-cartographer.mjs compare architecture base.json head.json \
...
```bash
node bin/code-cartographer.mjs visual-check web-app.html --json
```

This receipt measures bounded runtime behavior; it does not approve perceptual polish. Inspect the HTML or generated screenshots separately. Follow the [delivery contract](../code-cartographer/references/delivery-contract.md) when recording supplementary manual browser work; an unconstrained glance supports only perceptual review.

Use the delivery contract for the canonical browser-evidence coverage, artifact binding, visual-review status, and handoff fields. The [Skill contract](../code-cartographer/SKILL.md) explains the authoring invariants and the bounded repair loop.
