# Code Cartographer 下一轮稳定与惊喜切片：Deployment Ownership Contract

研究日期：2026-07-23（Asia/Shanghai）

Code Cartographer 已提交基线：[`codex/cursor-onboarding@a73047b`](https://github.com/tt-a1i/code-cartographer/tree/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782)
...
> 本文只把上述已提交版本作为 Code Cartographer 事实基线。研究期间工作区有其他任务并发修改，因此未提交内容不作为“已经具备”的证据。
...
它不增加第六种图、不增加第四套风格、不增加 Viewer 面板，也不引入云厂商图标。它把 Code Cartographer 已经能画、但目前只能靠作者自觉填写的四类事实变成可执行合同：
...
3. **以后再评估：Story-specific motion / GIF**；当前 Code Cartographer 已有有限 motion、Guided Story、WebM 与 README GIF，不应先扩第二套媒体系统。

## 1. Code Cartographer 真实基线：视觉证明已经有，语义合同还没有

Code Cartographer 当前已经有五种 typed renderer、三套同拓扑 preset、Guided Story、Finder、Focus、Route、Reach、Lens、Share / Route / Reach Card、WebM、revision-pinned source evidence、Atomic Delivery、Last-Good Preview、Structured Repair Receipt，以及 Cursor / Codex / Claude Code / OpenCode 共用的一个 Skill。当前 [README](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/README.md#L11-L24) 和 [PRODUCT](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/PRODUCT.md#L13-L37) 已把产品边界写得很清楚：交互必须来自 authored / verified evidence，默认交付仍是自包含文件。
...
这些事实可在当前 [production-deployment fixture](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/code-cartographer/examples/production-deployment.architecture.json#L1-L69) 中直接看到。
...
见当前 [Architecture schema](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/code-cartographer/schemas/architecture.schema.json#L8-L151)。所以当前状态是：**Code Cartographer 有一张部署归属样例，但没有能力保证下一张部署归属图仍然回答了同样的问题。** 这是本切片要关闭的真实缺口。
...
- GitHub stable `1.2.0` 与 npm legacy `1.0.4` 的发行漂移；Code Cartographer 的 ZIP、文档与实现必须由同一 gate 锁定。
...
- 入口必须容易记；Code Cartographer 当前 artifact footer → Start 页已经承担这件事，不需要新服务。
- profile 失败必须指出准确组件 / boundary / connection 与受支持修复，而不是泛化为“部署图不完整”。
- 1200×630 social card 是有效增长面；Code Cartographer 已经具备并应让新 proof 直接受益。GitDiagram 自身也用固定 1200×630 Open Graph card contract。[social image source](https://github.com/ahmedkhaleel2004/gitdiagram/blob/041d2feb4a9b1593dcf3bde2ca5b9ae7659becb9/src/server/og/cards.tsx#L5-L15)
...
- Mermaid 作为 Code Cartographer 的 canonical IR 或通用 auto-layout；两者产品边界不同。
...
这反而强化 Code Cartographer 的边界：Authored Reach 可以叫 authored reachability，不能借一个 deployment profile 偷换成 runtime impact、availability、failover correctness 或 blast radius。profile 只能验证 JSON 是否完整、自洽，不能证明云上真的这样部署。
...
- GitNexus 采用 PolyForm Noncommercial；这里只学习产品与测试模式，不搬代码进入 MIT Code Cartographer。
...
其中最值得 Code Cartographer 后续研究的是：
...
这些能力和 Code Cartographer 的 typed stable IDs、Share Card、revision evidence 很匹配，**Architecture Delta 是清晰的 P1**。
...
- 仅为“动起来”复制 looping marching ants。Code Cartographer 的 motion 必须有限、由读者控制、静态含义完整。
...
6. 重建 `code-cartographer.zip`，并让同一 package smoke 在 Ubuntu / macOS / Windows 跑最小静态子集；
...
但 Code Cartographer 必须先回答这些问题，才能不缩水：
...
- Code Cartographer committed baseline：[repository](https://github.com/tt-a1i/code-cartographer/tree/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782)、[README](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/README.md)、[Architecture schema](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/code-cartographer/schemas/architecture.schema.json)、[deployment proof source](https://github.com/tt-a1i/code-cartographer/blob/a73047b27e3b423fc8ab6ebd1ac84fd4ecb2e782/code-cartographer/examples/production-deployment.architecture.json)
