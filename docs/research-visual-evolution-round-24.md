...
an interactive, counted **Semantic Lens** over the semantics Code Cartographer already
compiled—not a generic database-style filter builder.

## Current Code Cartographer evidence

- [`focusNodeAttrs()`](../code-cartographer/renderers/shared/cli.mjs) already emits a stable
...
- The viewer already owns temporary focus, route, story, camera, URL, print, and
  export state in [`template.html`](../code-cartographer/assets/template.html). Lens belongs
...
Code Cartographer should borrow that interaction grammar, not G6's plugin/canvas stack or
...
Code Cartographer already has the simpler SVG equivalent: set viewer-only match/context
...
set visible, and offer an obvious reset. Code Cartographer does not need nested property
...
Code Cartographer is a fixed-geometry technical-diagram compiler. A semantic lens should
...
It is excessive for Code Cartographer. The compiled diagram is the authoritative view;
...
authoring burden. It turns Code Cartographer's existing colors and node kinds from passive
