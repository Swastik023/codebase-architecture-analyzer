...
Code Cartographer's story system can now explain the present unusually well. Story Follow
...
([viewer implementation](../code-cartographer/assets/template.html)).
...
node
([Story Follow contract](../code-cartographer/test/story-follow-camera.test.mjs)).
...
motion layer would compete with surfaces Code Cartographer already has. Story Horizon
...
**Adapt:** Code Cartographer should create expectation with one static immediate-next
...
**Adapt:** unlike a strict reveal, Code Cartographer should retain its stable topology and
...
**Adapt:** reuse Code Cartographer's existing `forward`, `reverse`, `multiple`, and
...
and composition semantics in Code Cartographer's source schema.
...
motion, and preserves Code Cartographer's strongest product boundary—one stable,
