...
numbers, while Code Cartographer does not yet serialize a route role rich enough to tell
...
Code Cartographer's current schemas; retain the three route budgets as metrics until
route-role calibration exists.
**Skip:** copying Fireworks' showcase numbers as immediate Code Cartographer failures or
...
**Adapt:** point to Code Cartographer's existing `via`, channel, side, and placement
...
- Border runs fail both Code Cartographer profiles; the Fireworks route-budget numbers
...
**A relationship may enter or leave a container, but Code Cartographer never lets the
...
because current Code Cartographer relationships target semantic nodes, not containers.
...
- Browser console logs contain no Code Cartographer runtime error.
