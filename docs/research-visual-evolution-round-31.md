...
chapter, Code Cartographer should show which stable node IDs would **stay in focus**, **enter
...
No inspected primary source ships this exact Code Cartographer interaction as a package.
...
[`Code Cartographer.guidedViews`](../code-cartographer/assets/template.html#L5424-L6252):
...
is inspected. **Adapt:** Code Cartographer needs one preview generation over stable SVG IDs,
...
would duplicate rendering and hide the precise semantic delta Code Cartographer can provide.
...
matters. **Adapt:** derive membership from independent Code Cartographer `focus` arrays; do
...
| Decision | Code Cartographer contract |
|---|---|
...
10. Focus, hover, and Escape do not write `activeIndex`, `aria-current`,
    `aria-pressed`, URL/hash/history, `Code Cartographer.focus`, Story Trail data,
...
Keep this inside the existing template-level `Code Cartographer.guidedViews` module so all
