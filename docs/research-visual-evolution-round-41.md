...
Build **Settled Flow**: an ordinary trace-enabled Code Cartographer artifact should run
...
Code Cartographer already has Story Trail, Story Beats, Story Follow Camera, Story
...
`code-cartographer/assets/template.html` currently gives trace edges and nodes:

```css
animation: code-cartographer-edge-flow 2.4s linear infinite;
animation: code-cartographer-node-pulse 3.6s ease-in-out infinite;
```
...
source topology.
**Adapt:** Code Cartographer can reuse its existing CSS markers and six-second WebM
...
**Adapt:** Code Cartographer's Story features already own deliberate step navigation, so
...
model.
**Adapt:** treat Code Cartographer animation state as viewer-only; authored classes and
...
legible settled topology, with source geometry and meaning preserved. Code Cartographer
