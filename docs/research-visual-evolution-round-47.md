...
Code Cartographer already has stronger semantic exploration, guided stories, export, and
...
What can Code Cartographer learn from `fireworks-tech-graph` without copying its breadth
...
3. The same reference contains a large product/brand catalogue. That breadth is
   ...
   matching responsibilities that Code Cartographer does not need for its core promise.
...
## Decision: Code Cartographer Semantic Sigils
...
## Why this adaptation fits Code Cartographer
...
- Twelve generic selectable skins: Code Cartographer keeps three coherent presets.
...
- Style-specific looping signatures for every scene: Code Cartographer already uses
