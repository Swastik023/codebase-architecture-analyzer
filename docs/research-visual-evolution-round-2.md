# Code Cartographer visual evolution research — round 2

Research date: 2026-07-19 (Asia/Shanghai)

## Executive decision

Code Cartographer should absorb **viewer intelligence**, **stable semantic identity**, and
...
associated with graph canvases without turning Code Cartographer into one.

## Round 2 implementation status

The `codex/code-cartographer-visual-evolution` branch now implements the first candidate
...
checks keep it inside Code Cartographer's read-only viewer boundary.
...
- Motion is deliberately constrained by semantic roles and scene contracts,
  but its optional GIF path is a substantially heavier stack than Code Cartographer's
...
- a scenario suitability guide that tells users which Code Cartographer renderer and
...
- drag, resize, connect, lasso, whiteboard, collaborative editing, and a React
  runtime. Those solve canvas-authoring problems outside Code Cartographer's boundary.
...
- Its default visual renderer uses the browser Canvas API. That enables large,
  rich graphs, but Canvas does not itself give Code Cartographer the semantic DOM and
...
- Its extension surface includes minimaps, fisheye, tooltips, history, timebars,
  ...
  but also evidence that the full engine is much broader than Code Cartographer needs
...
- embedding G6 or its plugin ecosystem. Code Cartographer can implement the small,
...
- the conceptual split between an overview, alternate scenario, and ordered
  walkthrough; use it to shape Code Cartographer recipes and examples.
...
- Mermaid is embeddable through ESM/CDN and offers a smaller “tiny” build, but
  the full package has a broad runtime dependency graph. Code Cartographer should copy
...
- The JavaScript distribution supports Web Workers and ships a browser-ready
  ...
  Code Cartographer's current renderer-specific geometry
...
| Capability | Best evidence | Code Cartographer interpretation |
|---|---|---|
...
> Code Cartographer compiles typed technical intent into a deterministic, validated,
