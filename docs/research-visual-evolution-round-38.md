...
look?” but not always “what happened here?” Code Cartographer already owns the facts needed
...
Code Cartographer has already solved the surrounding navigation layers:
...
**Adapt:** Code Cartographer should express semantic specificity as exact, readable beat
...
project, not general facts Code Cartographer can safely infer from its five typed IRs.
...
workspace navigation model. Code Cartographer's full static truth and local framing are
...
**Adapt now:** derive a concise explanation only from Code Cartographer's existing edge
...
**Adapt:** retain Code Cartographer's current pause-on-focus behavior and Live/Still owner.
...
| Per-diagram packet/train/ECG motion signatures | High spectacle | Low chrome, very high runtime surface | Needs explicit semantics Code Cartographer does not currently own | **Adapt later** |
...
feature has violated Code Cartographer's trust boundary. Fail closed to relation class,
