# Code Cartographer first-class Cursor onboarding research

> Snapshot: 2026-07-23. Sources are limited to Cursor's official documentation and changelog, the official `vercel-labs/skills` repository and CLI, Code Cartographer's own issue/docs/tests, and first-party peer repositories.

## Decision

**Recommendation:** add Cursor as a first-class installation surface for the existing `code-cartographer` Skill. Do not fork `SKILL.md`, add a Cursor-only runtime, or call the work complete until one real Cursor Editor/CLI session discovers the Skill and produces a verified artifact.
...
npx -y skills add tt-a1i/code-cartographer --skill code-cartographer --agent cursor --copy --yes
```

For a reproducible test fixture, pin the version that was verified in this snapshot:

```bash
npx -y skills@1.5.20 add tt-a1i/code-cartographer --skill code-cartographer --agent cursor --copy --yes
```
...
| Scope | Valid Skill roots | Code Cartographer entry |
|---|---|---|
| Project | `.agents/skills/`, `.cursor/skills/` | `<root>/code-cartographer/SKILL.md` |
| User/global | `~/.agents/skills/`, `~/.cursor/skills/` | `<root>/code-cartographer/SKILL.md` |
...
### Code Cartographer's current evidence

- [Issue #46](https://github.com/tt-a1i/code-cartographer/issues/46) is open and asks to collaborate on Cursor usage. It had no comments at this snapshot.
- The current Code Cartographer README names Claude, Codex CLI, and opencode, and its installation matrix does not include Cursor.[Current positioning](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L11-L19) · [Current installation matrix](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/README.md#L223-L231)
- Code Cartographer requires Node.js 18 or newer, but its distributed renderer and validators do not require `npm install`.[Runtime contract](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/code-cartographer/package.json#L8-L13) · [Skill setup contract](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/code-cartographer/SKILL.md#L27-L33)
- Existing tests already simulate a copied installation with `node_modules` and repository-only tests removed, then exercise all five modes through `deliver` and run live preview.[Installed-copy fixture](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/code-cartographer/test/cli.test.mjs#L56-L69) · [Five-mode installed delivery](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/code-cartographer/test/cli.test.mjs#L357-L381) · [Installed preview](https://github.com/tt-a1i/code-cartographer/blob/e7e22a1e3243d51c1cfbfcce005db1d3355478b3/code-cartographer/test/cli.test.mjs#L383-L425)
...
```bash
npx -y skills@1.5.20 add tt-a1i/code-cartographer \
  --skill code-cartographer --agent cursor --copy --yes
```

Observed evidence:

1. `npx -y skills@1.5.20 list --agent cursor --json` reported one project-scoped `Cursor` installation at `.agents/skills/code-cartographer`.
2. `node .agents/skills/code-cartographer/bin/code-cartographer.mjs doctor` exited 0.
...
|---|---|---|---|
| Global | Current CLI resolves Cursor to `~/.agents/skills/code-cartographer`; Cursor officially scans that root. | A shell-capable Skill becomes available in every project; update timing is user-wide. | Default for an individual who wants Code Cartographer everywhere. Say exactly what scope it grants. |
| Project | Installs `.agents/skills/code-cartographer` plus `skills-lock.json` in the current repository. | The copied Skill is a substantial repository diff if committed, and can become stale relative to upstream. | Offer as the team/reproducible option, not as a silent default. |
...
- “Install Code Cartographer for Cursor with `--agent cursor`; use `.agents/skills/code-cartographer` for a project or a Cursor-supported global Skill root for user scope.”
- “The installed Code Cartographer package passes `doctor`, validation, and checked HTML delivery without `npm install`, provided Node.js 18+ is available.”
- “Code Cartographer uses the same portable `SKILL.md` and renderer contract across Cursor, Claude, Codex, and opencode.”
...
- “The global CLI command installs to `~/.cursor/skills/code-cartographer`.” The current implementation resolves Cursor as universal and may use `~/.agents/skills/code-cartographer`.
- “Cursor's Remote Rule UI imports Code Cartographer correctly from the bare repository URL.” Cursor documents the UI, but not nested Skill selection; Code Cartographer's actual Skill lives under `code-cartographer/`.
...
3. Reuse the same `code-cartographer/SKILL.md`; add no Cursor fork or Cursor-specific diagram behavior.
