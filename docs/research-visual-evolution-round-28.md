...
Code Cartographer already compiles authored `guidedViews`, renders the active chapter's
...
[`Code Cartographer.guidedViews`](../code-cartographer/assets/template.html) parses the existing
`code-cartographer-guided-views-data` payload into an ordered `views` array. Every entry
already has:
...
direct live-SVG serialization as Code Cartographer's export implementation. Code Cartographer's
...
selection. Code Cartographer already has deliberate Play/Pause ownership and should not
expand it.
...
ambient rotation, or full-page layout. The Code Cartographer rail must work by explicit
...
or a current-viewport export option in this slice. The current Code Cartographer
...
3. Do not change `Code Cartographer.exporter`, export cleanup, canonicality receipts,
...
boundaries keeps the result attractive, stable and recognizably Code Cartographer.
